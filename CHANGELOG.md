...
- Erstellen Sie ein React-Projekt mit create-vite: `npm init --yes vite@latest gasbetfrontend -- --template=react-ts  --no-rolldown --no-immediate`..
- `vite.config.ts` wird mit dem Port aktualisiert.
- Projektdatei (`gasbetfrontend.esproj`) erstellen.
